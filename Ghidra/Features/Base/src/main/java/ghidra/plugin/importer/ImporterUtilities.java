...
public class ImporterUtilities {
    ...
    private static void importFromContainer(PluginTool tool, ProgramManager programManager,
        ...
        // Atomic references for the lambdas
        final AtomicReference<FileSystemBrowserService> fsbService = new AtomicReference<>();
        final AtomicInteger choice = new AtomicInteger(0); // cancelled

        tool.getService(FileSystemBrowserService.class).ifPresentOrElse(
                service -> {
                    int option = OptionDialog.showOptionDialog(parent, "Container File Detected",
        ...
        switch (choice.get()) {
            case 1 -> importSingleFile(fullFsrl, destinationFolder, suggestedPath, tool, programManager,
                    monitor);
            case 2 -> BatchImportDialog.showAndImport(
                    tool,
                    null,
                    Collections.singletonList(fullFsrl),
                    destinationFolder,
                    programManager
            );
            case 3 -> {
                final FileSystemBrowserService service = fsbService.get();
                if (service != null) {
                    service.openFileSystem(fullFsrl);
                }
            }
            default -> {
                // 0 = cancelled.
    ...
    public static void showAddToProgramDialog(FSRL fsrl, Program program, PluginTool tool,
        ...
        try {
            ...
            LoaderMap loaderMap = LoaderService.getSupportedLoadSpecs(provider, Loader::supportsLoadIntoProgram);
