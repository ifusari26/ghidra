...
public class ImporterPlugin extends Plugin
    ...
    protected void init() {
        ...
        tool.getService(FrontEndService.class).ifPresent(service -> {
    ...
    @Override
    public void importFiles(@Nullable final DomainFolder destFolder, final List<File> files) {
        final AtomicReference<DomainFolder> domainFolder = new AtomicReference<>();
        Optional.ofNullable(destFolder).ifPresentOrElse(
                domainFolder::set,
                () -> {
                    final DomainFolder rootFolder = tool.getProject().getProjectData().getRootFolder();
                    domainFolder.set(rootFolder);
                }
        );
        // Unpack database.
        files.addAll(handleSimpleDBUnpack(domainFolder.get(), files));
        if (files.isEmpty()) {
            return;
        }
        getTool().getService(ProgramManager.class)
                .ifPresent(
                        service -> BatchImportDialog
                                .showAndImport(tool, null, files2FSRLs(files), domainFolder.get(), service)
                );
    }
    ...
    @Override
    public void importFile(final DomainFolder folder, final File file) {
        final AtomicReference<DomainFolder> domainFolder = new AtomicReference<>();
        Optional.ofNullable(folder).ifPresentOrElse(
                domainFolder::set,
                () -> {
                    final DomainFolder rootFolder = tool.getProject().getProjectData().getRootFolder();
                    domainFolder.set(rootFolder);
        ...
        if (handleSimpleDBUnpack(domainFolder.get(), file)) {
            return;
        }
        tool.getService(ProgramManager.class).ifPresent(
                service -> {
                    FSRL fsrl = FileSystemService.getInstance().getLocalFSRL(file);
                    ImporterUtilities.showImportDialog(tool, service, fsrl, domainFolder.get(), null);
        ...
        batchImportAction = new DockingAction(title, this.getName()) {
            ...
            public void actionPerformed(ActionContext context) {
                tool.getService(ProgramManager.class).ifPresent(
                        service -> BatchImportDialog
                                .showAndImport(
                                        tool, null, null, getFolderFromContext(context), service
                                )
                );
            }
        ...
        tool.getService(ProgramManager.class).ifPresent(
                service -> {
                    ...
                    TaskLauncher.launchModal(
                            "Show Add To Program Dialog",
                            monitor -> ImporterUtilities.showAddToProgramDialog(fsrl, program, tool, monitor)
                    );
                }
        );
    }
    ...
    protected void doImportSelectionAction(Program program, ProgramSelection selection) {
        ...
        try {
            ...
            tool.getService(ProgramManager.class).ifPresent(
                    service -> {
                        final ImporterDialog dialog = new ImporterDialog(tool, service, loaderMap, bp, null);
