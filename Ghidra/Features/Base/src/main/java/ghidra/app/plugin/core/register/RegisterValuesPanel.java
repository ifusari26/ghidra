...
class RegisterValuesPanel extends JPanel {
    ...
    private GhidraTable createBasicTable() {
        ...
        tool.getService(GoToService.class).ifPresent(service -> {
            table.installNavigation(service, service.getDefaultNavigatable());
    ...
    void updateMarkers() {
        ...
        tool.getService(MarkerService.class).ifPresent(service -> {
            if (markerSet == null) {
                markerSet = service.createAreaMarker(
                        "Register Values",
    ...
    private void clearMarkers(Program program) {
        // Already cleared / program does not exist?
