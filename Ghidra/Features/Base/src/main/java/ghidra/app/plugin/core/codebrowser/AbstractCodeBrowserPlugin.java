...
public abstract class AbstractCodeBrowserPlugin<P extends CodeViewerProvider> extends Plugin
    ...
    @Override
    protected void init() {
        tool.getService(MarkerService.class)
                .ifPresent(service -> {
        ...
        if (viewManager == null) {
            tool.getService(ViewManagerService.class).ifPresent(service -> viewManager = service);
        }

        tool.getService(ClipboardService.class)
                .ifPresent(service -> {
    ...
    public P createNewDisconnectedProvider() {
        ...
        tool.getService(ClipboardService.class).ifPresent(newProvider::setClipboardService);
    ...
    public void serviceAdded(Class<?> interfaceClass, Object service) {
        ...
        if (interfaceClass == MarkerService.class && markerService == null) {
            tool.getService(MarkerService.class).ifPresent(srv -> {
                markerService = srv;
                markerService.addChangeListener(markerChangeListener);
            });
            updateBackgroundColorModel();
