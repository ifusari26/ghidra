...
public class OperandFieldMouseHandler implements FieldMouseHandlerExtension {
    ...
    private boolean checkOperandFieldLocation(Navigatable navigatable, CodeUnit codeUnit,
        ...
        return serviceProvider
                ...
                .map(service -> {
                    ...
                    if (checkVariableReference(navigatable, codeUnit, loc, service) ||
                            checkExternalReference(navigatable, codeUnit, loc, service) ||
                            checkMemRefs(navigatable, codeUnit, loc, serviceProvider)) {
                        return true;
                    }

                    return checkOpObject(navigatable, codeUnit, opIndex, loc.getSubOperandIndex(), service);
                }).orElse(false);
    }
    ...
    private boolean checkMemRefs(Navigatable navigatable, CodeUnit codeUnit,
       ...
       return serviceProvider.getService(GoToService.class)
			   .map(
			   		service -> service.goTo(navigatable, codeUnit.getProgram(), gotoAddr, codeUnit.getAddress())
	   			)
			   .orElse(false);
    }
    ...
    private boolean followIndirectReference(ServiceProvider serviceProvider,
        ...
        return serviceProvider.getService(OptionsService.class).map(service -> {
			NavigationOptions navOptions = new NavigationOptions(service);
			try {
