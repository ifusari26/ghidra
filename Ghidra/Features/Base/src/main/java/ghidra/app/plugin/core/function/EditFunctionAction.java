    ...
    public void actionPerformed(ProgramActionContext context) {
        ...
        if (context instanceof ListingActionContext) {
            ...
            if (loc instanceof FunctionLocation) {
                function = Optional.ofNullable(functionPlugin.getFunction(listingContext));
            } else if (loc instanceof OperandFieldLocation) {
                function = Optional.ofNullable(functionPlugin.getFunctionInOperandField(context.getProgram(),
            ...
            function = Optional.ofNullable((Function) symbol.getObject());
        }
        // Function
        if (function.isPresent()) {
            PluginTool tool = functionPlugin.getTool();
            tool.getService(DataTypeManagerService.class).ifPresent(service -> {
                FunctionEditorDialog dialog = new FunctionEditorDialog(service, function.get());
