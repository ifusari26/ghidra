...
public abstract class AbstractGhidraHeadlessIntegrationTest extends AbstractDockingTest {
    ...
    public void goTo(PluginTool tool, Program p, Address addr) {
        tool.getService(GoToService.class).ifPresentOrElse(
                service -> service.goTo(addr),
                () -> tool.firePluginEvent(new ProgramLocationPluginEvent("Test", new ProgramLocation(p, addr), p))
        );
        waitForSwing();
    }
    ...
    public static <T> void replaceService(PluginTool tool, Class<? extends T> service,
        ...
        List<Class<?>> usedExtensions = new ArrayList<>();
        List<Class<?>> extensionPoints =
                (List<Class<?>>) getInstanceField("extensionPoints", ClassSearcher.class);
        Set<Class<?>> extensions = new HashSet<>(extensionPoints);
        extensions.forEach(extension -> {
            if (service.isAssignableFrom(extension)) {
                usedExtensions.add(extension);
                tool.getService(service).ifPresent(instance -> serviceManager.removeService(service, instance));
            }
        });

        // Remove all extensions associated with the service.
        usedExtensions.forEach(extensions::remove);

        // Actual replacement operation.
        tool.getService(service).ifPresent(instance -> {
            // Remove.
            serviceManager.removeService(service, instance);
            // Replace
            extensions.add(replacement.getClass());
        });
        // Handle with the ServiceManager
        serviceManager.addService(service, replacement);

        // Replace reflectively
        List<Class<?>> newExtensionPoints = extensions.stream().toList();
